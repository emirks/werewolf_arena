...
class LiveKitParams(TransportParams):
    audio_in_participant_ids: Optional[List[str]] = None
    "If set, only process audio from these participant identities."
...
class LiveKitTransportClient:
    ...
    async def _async_on_track_subscribed(
        ...
        if track.kind == rtc.TrackKind.KIND_AUDIO:
            # If a list of participant IDs is provided, only process audio from them.
            if self._params.audio_in_participant_ids != None and participant.identity not in self._params.audio_in_participant_ids:
                # logger.debug(f"Skipping audio processing for participant {participant.identity}")
                return

            logger.info(f"Audio track subscribed: {track.sid} from participant {participant.identity}")
