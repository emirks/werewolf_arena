...
class TranscriptionProcessor(FrameProcessor):
    ...
    async def _handle_transcription_complete_frame(self, frame: TranscriptionCompleteFrame):
...
class DataChannelProcessor(FrameProcessor):
    ...
    async def _handle_data_frame(self, frame: DataFrame):
        ...
        try:
            ...
            elif message_type == 'target_selection' and self._on_target_selection_received:
                target = message.get('target')
                if target:
            ...
            if normalized_rms > self._speech_threshold:
                ...
                if not self._is_speaking:
                    ...
                    await self.push_frame(
                        SpeechStateFrame(is_speaking=True), 
                ...
                if (self._is_speaking and 
                    current_time - self._last_speech_time > self._silence_duration_ms):
                    
                    self._is_speaking = False
                    # Emit speech end frame
                    await self.push_frame(
                        SpeechStateFrame(is_speaking=False), 
                        FrameDirection.DOWNSTREAM
                    )
